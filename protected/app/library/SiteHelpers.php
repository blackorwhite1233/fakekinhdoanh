...
class SiteHelpers
{
	...
	public static function getCMND($cmnd){
		$arr_cmnd = str_split($cmnd);
		$limit = count($arr_cmnd) - 2;
		$rs = '';
		for($i=0;$i<$limit;$i++){
			$rs .= $arr_cmnd[$i];
		}
	...
	public static function getNameUser($id = ''){
		$user = DB::table('customer')->where("customer_id","=",$id)->first();
		return $user->name;
	}
	...
	public static function templatePost($data = "",$type = 'user')
	{
		...
		$output .= '<div class="image"><a href="'.$link.'"><img src="'.$image.'" title="'.$data->post_name.'" alt="'.$data->post_name.'"></a></div>';
	    $output .= '<div class="name"><a href="'.$link.'">'.$data->post_name.'</a></div>';
	    $output .=  '<div class="price">'.$price.' VNĐ</div>';
	    $output .=  $promotion;
	    $output .=  '<div class="contact"><i class="fa fa-phone"></i><span>'.$data->phone.'</span></div>';
        $output .=  '</div>';
        return $output;
	}
